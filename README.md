...
- auth-service — authentication service responsible for managing user permissions
...
Let’s update the bootstrap() function boilerplate code in the main.ts file of the auth-service app with the NestFactory.createMicroservice() method:
...
```ts
// apps/auth-service/src/app/user.repository.ts
